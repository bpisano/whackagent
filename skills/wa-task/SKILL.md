...
5. **Add to backlog.** Append task under **Todo** in `{backlog}`, link file (relative to the backlog's own folder, so the link works when backlog and tasks sit in different trees).
...
5. **Reorder.** Order in **Todo** section = priority (top = next). No numeric labels written in `{backlog}` — order alone carry priority. Reflect new order in `{backlog}`.
